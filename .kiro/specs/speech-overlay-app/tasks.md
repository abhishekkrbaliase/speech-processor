...
- [x] 2. Implement basic window management and overlay functionality
  ...
  - [ ] 2.3 Fix electron-builder packaging configuration and build pipeline
    - Resolve file structure conflicts between webpack output and electron-builder expectations
    - Streamline build pipeline to eliminate manual HTML file copying requirements
    - Fix path resolution issues in electron-builder configuration for proper asset bundling
    - Optimize webpack configuration to output files in electron-builder compatible structure
    - Create reliable platform-specific packaging (separate Windows and macOS packages are acceptable)
    - Ensure packaged applications launch and run correctly on target platforms without configuration
...
- [ ] 3. Implement data management and file handling
  - [x] 3.1 Create CSV parser for patient data
  ...
  - [ ] 3.2 Implement questions file loader
...
- [ ] 4. Integrate Google Speech-to-Text API for speech processing
  - [ ] 4.1 Set up Google Cloud Speech-to-Text client integration
    - Install and configure Google Cloud Speech-to-Text client library (@google-cloud/speech)
    - Create GoogleSpeechService class for API lifecycle management
    - Implement API client initialization with credential management
    - Add error handling for authentication and network issues
    - _Requirements: 6.4, 6.5, 6.6, 4.7_
